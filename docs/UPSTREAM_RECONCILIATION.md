...
## Retained local-window attention

The released VDN local-window operator is exact SDPA. Model-level `transformer_options` attention overrides apply to the native/base attention path and must not leak into VDN's retained local-window branch. v1.5.2 therefore accepts the options argument for API compatibility but deliberately passes `None` to every local `_sdpa` call. Regression coverage verifies exact grouped-window numerical parity and no Sage/Kitchen/model-level override leakage.
...
The final v1.5.2 PR head passed CI run 232. The corrected complete production RTX PRO 6000 workflow was then re-run on current ComfyUI with the intended Untwist RoPE configuration and completed without the CUDA abort; decoded video quality was reviewed as normal/artifact-free. That closes the GPU correctness release gate. CPU/oracle CI remains the source of numerical/ownership contract evidence, while no new performance claim is made without matched GPU measurement.
