...
_ATTN_OUT = ("attn.orig.to_out.0", "attn.to_out.0")


def _comfy_path(key, is_refiner):
    """diffusers/hybrid module path -> (comfy module path, conversion kind).

    OpenVDN Stage-B adapters can name hybrid attention through ``attn.orig.*`` while
    external/Turbo adapters are commonly trained against the dense model and use
    ``attn.*`` directly. Token-refiner attention also uses the dense spelling. All of
    those source names must map onto ComfyUI's fused ``qkv_proj``/``out_proj`` layout.
    """
    if is_refiner:
    ...
    for proj in _ATTN_OUT:
        if stem.endswith("." + proj):
            return stem[: -len(proj)] + "attn.out_proj", "out"
...
def convert_adapter(sd, adapter_cfg):
    ...
    for path, group in qkv_groups.items():
        ...
        common_scale = scales[0] if common else 1.0
