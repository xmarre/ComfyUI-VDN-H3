...
def test_qkv_fusion_preserves_common_alpha_scale(alpha):
    ...
    for suffix in ("to_q", "to_k", "to_v"):
        ...
        b = torch.randn(out, rank)
        state.update(_pair(module, a, b))
        individual.append((b.float() @ a.float()) * (alpha / rank))

    converted = convert_adapter(state, {"rank": rank, "alpha": alpha})
    a, b, scale = converted["blocks.0.attn.qkv_proj"]
    assert scale == alpha / rank
...
def test_dense_model_attention_names_map_to_fused_comfy_layout():
    torch.manual_seed(422)
    rank, inp, out = 2, 6, 5
    state = {}
    individual = []
    for suffix in ("to_q", "to_k", "to_v"):
        module = f"transformer_blocks.1.attn.{suffix}"
        a = torch.randn(rank, inp)
        b = torch.randn(out, rank)
        state.update(_pair(module, a, b))
        individual.append(b.float() @ a.float())

    out_a = torch.randn(rank, out)
    out_b = torch.randn(inp, rank)
    state.update(_pair("transformer_blocks.1.attn.to_out.0", out_a, out_b))
    ...
    fused_a, fused_b, fused_scale = converted["blocks.1.attn.qkv_proj"]
    assert fused_scale == 1.0
    assert torch.allclose(
        fused_b.float() @ fused_a.float(),
        torch.cat(individual, dim=0),
        atol=2e-6,
        rtol=2e-6,
    )
    got_a, got_b, got_scale = converted["blocks.1.attn.out_proj"]
    assert torch.equal(got_a, out_a)
    assert torch.equal(got_b, out_b)
    assert got_scale == 1.0


def test_token_refiner_attention_maps_to_fused_comfy_layout():
    ...
    out_a = torch.randn(rank, out)
    out_b = torch.randn(inp, rank)
    state.update(_pair("token_refiner.refiner_blocks.0.attn.to_out.0", out_a, out_b))
    ...
    fused_a, fused_b, fused_scale = converted["token_refiner.blocks.0.attn.qkv_proj"]
    assert fused_scale == 1.0
    assert torch.allclose(
        fused_b.float() @ fused_a.float(),
        torch.cat(individual, dim=0),
    ...
    got_a, got_b, got_scale = converted["token_refiner.blocks.0.attn.out_proj"]
    assert torch.equal(got_a, out_a)
    assert torch.equal(got_b, out_b)
    assert got_scale == 1.0
